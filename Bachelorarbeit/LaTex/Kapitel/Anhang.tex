...
\begin{algorithm}[H]
\caption{Ablauf eines Requests an den Job Status Endpoint}\label{appendixpsedoProcessDescription}
\scriptsize
\begin{algorithmic}     
    \STATE HTTP-Methode $\gets$ Anfrage.HTTP-Methode
    \STATE Encoding $\gets$ Anfrage.f
    ...
    \ELSE{}
        ...
        \begin{ALC@g}
            \IF{Encoding == text/html \OR Encoding == None}
                \IF{templates/json/processes/Job-ID.json exists}
                    \STATE JSON $\gets$ open(jobs/Job-ID/status.json)
                    \STATE Response $\gets$ render\_template(templates/html/job.html, JSON)
                    \STATE Link-Header $\gets$ http://HOST:PORT/jobs/<jobID>?f=text/html
            ...
            \ELSIF{Encoding == application/json}
                ...
                \STATE JSON $\gets$ open(jobs/Job-ID/status.json)
                    \STATE Response $\gets$ jsonify(JSON)
                    \STATE Link-Header $\gets$ http://HOST:PORT/jobs/<jobID>?f=application/json
                    \STATE Resource-Header $\gets$ job - Job-ID
                    \RETURN{HTTP-Statuscode 200 \AND Antwort mit Link- und Resource-Header}
                \ELSE{}
                    \STATE Exception $\gets$ No such job exception
