...
Reflektivität ab und haben eine annähern quadratische geometrische Auflösung. Die Reflektivität wird in der logarithmischen Maßeinheit Decibel (dB) angegeben. Die Korrektur 
der Schrägdistanz in Blickrichtung erfolgt dabei erfolgt dabei durch Projektion auf einen Ellipsoiden. \cite{sentinel_1_definition}. Aus den Level-1 Produkten können die 
Level-2 Produkte OSW, OWI und RVL abgeleitet werden.

\subsubsection{Datenzugang}
\subsection{Überschwemmungsmonitoring}
Um Wasserflächen und damit auch überflutete Areale auf Radarbildern zu erkennen können die Reflektionseingenschaften von Wasserflächen genutzt werden. Das Wasser eine 
sehr niedrige Rauigkeit besitzt kommt beim Aufprall eines Radarsignals zu einer Spiegelung und nur ein sehr geringer Teil des Signals wird zum Empfänger 
zurückgeworfen. In den resultierenden Bildern äußert sich dieser Umstand in niedrigen Refelktivitätswerten. 
Um die Areale mit niedrigen Reflektionswerten automatisiert detektieren können Verfahren genutzt werden welche aus den Histogrammen der Bilder einen Schwellwert berechnen.
...
Jeder dieser Werte teilt das Histogramm in zwei Gruppen und bildet so einen Schwellwert. 
